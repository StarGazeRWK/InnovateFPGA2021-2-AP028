...
//          GPIO_1_input
//=========================================================
void *h2p_lw_gpio1_input_addr;
int input_button=0x00;
int startButton_Mask = 0x01;
int stopButton_Mask = 0x02;

//=========================================================
//          ARDUINO
//=========================================================
void *h2p_lw_arduino_addr;
int arduino_mask_temp;

//=========================================================
//          GPIO_0_output
//=========================================================
void *h2p_lw_gpio0_output_addr;
int gpio0_mask;
...
int boardInitialization() {
	...
	//GPIO0 Output
	h2p_lw_gpio0_output_addr=virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + GIPIO0_OUTPUT_PIO_BASE ) & ( unsigned long)( HW_REGS_MASK ) );
	
	//GPIO1 Output
	h2p_lw_gpio1_output_addr=virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + GPIO1_PIO_BASE ) & ( unsigned long)( HW_REGS_MASK ) );
	
	//GPIO1 Input
	h2p_lw_gpio1_input_addr=virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + GPIO1_INPUT_PIO_BASE ) & ( unsigned long)( HW_REGS_MASK ) );
	
	//ARDUINO DHT22
	h2p_lw_arduino_addr = virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + XXXXX ) & ( unsigned long)( HW_REGS_MASK ) );
...
{ 
  input = *(uint32_t *)h2p_lw_arduino_addr; 
  return(input);
}
...
void writeFPGA_GPIO_0_outputRGB(int gpio0_mask_temp)			
{ *(uint32_t *)h2p_lw_gpio0_output_addr = gpio0_mask_temp; }

void writeFPGA_ARDUINO_DHT(int arduino_mask_temp)
{ *(uint32_t *)h2p_lw_arduino_addr = arduino_mask_temp; }
